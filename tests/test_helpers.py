...
def assert_objeto_possui_atributos(objeto: Any, atributos: Dict[str, Any]):
    ...
    for nome_attr, valor_esperado in atributos.items():
        ...
        ), f"Atributo '{nome_attr}': esperado {valor_esperado}, obtido {valor_atual}"


def assert_listas_equivalentes(
    lista1: List[Any], lista2: List[Any], comparar_por: str = "id"
):
...
def assert_usuario_valido(usuario):
    ...
    assert "@" in usuario.email, "E-mail deve ter formato válido"
...
def assert_categoria_valida(categoria):
    ...
    assert hasattr(
        categoria, "tipo_fornecimento"
    ), "Categoria deve ter tipo_fornecimento"
...
class AssertHelper:
    ...
    @staticmethod
    def todos_elementos_sao_tipo(
        lista: List[Any], tipo_esperado: type, nome_lista: str = "lista"
    ):
        ...
        for i, elemento in enumerate(lista):
            ...
            ), f"Elemento {i} de {nome_lista} deve ser do tipo {tipo_esperado.__name__}"
    ...
    def ids_unicos(lista: List[Any], nome_lista: str = "lista"):
        ...
        assert len(ids_nao_nulos) == len(
            set(ids_nao_nulos)
        ), f"IDs em {nome_lista} devem ser únicos"
    ...
    def emails_unicos(lista: List[Any], nome_lista: str = "lista"):
        ...
        assert len(emails_nao_nulos) == len(
            set(emails_nao_nulos)
        ), f"Emails em {nome_lista} devem ser únicos"
