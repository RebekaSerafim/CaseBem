...
def register_failed_attempt(ip: str):
    ...
    if failed_attempts[ip] >= MAX_FAILED_ATTEMPTS:
        ...
        security_logger.warning(
            f"IP {ip} bloqueado após {MAX_FAILED_ATTEMPTS} tentativas falhadas"
        )
...
def log_security_event(
    event_type: str, request: Request, usuario: dict = {}, details: str = ""
):
...
def enhanced_create_session(request: Request, usuario: dict) -> None:
    ...
    from infrastructure.security.auth_decorator import criar_sessao
...
def security_middleware():
    ...
    async def middleware(request: Request, call_next):
        ...
        if is_ip_blocked(client_ip):
            log_security_event(
                "BLOCKED_IP_ACCESS", request, details=f"IP {client_ip} está bloqueado"
            )
            return RedirectResponse(
                url="/blocked", status_code=status.HTTP_403_FORBIDDEN
            )
