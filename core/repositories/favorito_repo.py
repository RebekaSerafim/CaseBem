...
class FavoritoRepo(BaseRepo):
    ...
    def __init__(self):
        super().__init__(
            nome_tabela="favorito", model_class=Favorito, sql_module=favorito_sql
        )
    ...
    def adicionar(self, id_noivo: int, id_item: int) -> bool:
        ...
        favorito = Favorito(id=0, id_noivo=id_noivo, id_item=id_item, data_adicao=None)
    ...
    def obter_por_noivo(self, id_noivo: int) -> List[dict]:
        ...
        resultados = self.executar_consulta(
            favorito_sql.OBTER_FAVORITOS_POR_NOIVO, (id_noivo,)
        )
    ...
    def verificar(self, id_noivo: int, id_item: int) -> bool:
        ...
        resultados = self.executar_consulta(
            favorito_sql.VERIFICAR_FAVORITO, (id_noivo, id_item)
        )
