#!/usr/bin/env python3
"""
Script para exportar dados das tabelas orcamento e item_orcamento para JSON
"""
import sqlite3
import json
...
def main():
    """Executa a exportação das tabelas orcamento e item_orcamento"""
    ...
    # Exportar tabelas
    total_orcamentos = export_table_to_json(
        DB_PATH,
        "orcamento",
        SEEDS_PATH / "orcamentos.json"
    )

    total_itens = export_table_to_json(
        DB_PATH,
        "item_orcamento",
        SEEDS_PATH / "itens_orcamentos.json"
    )

    print(f"\n🎉 Exportação concluída!")
    print(f"   - {total_orcamentos} orçamentos")
    print(f"   - {total_itens} itens de orçamento")
